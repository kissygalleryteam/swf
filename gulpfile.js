    ...
    Object.keys(excludes).forEach(function (tag) {
        ...
        tasks.push(function (done) {
            gulp.src('./lib/' + tag + '.js')
				...
				.pipe(replace(/modulex\.add\("swf(.)/, function(nul, match) {
					console.log(match);
					if ('"' === match) {
						return 'define("kg/swf/0.0.1/index"';		
					}
					return 'define("kg/swf/0.0.1' + match;		
				 }))
				.pipe(replace(/modulex.config\(([^)]+)\)/g, function(nul, match) {
					return 'KISSY.config({' + match.replace(/"requires"\s*,/, '"modules":') + '});';	  
				 }))
				 .pipe(rename(function(path) {
					path.basename = path.basename.replace('-debug', '').replace(/swf$/, 'index').replace(/swf-deps$/, 'index-deps');	
				 }))
